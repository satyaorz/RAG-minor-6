...
\IEEEauthorblockN{
\begin{tabular}{cc}
Dr. Santosh Kumar & Satya Pradhan \\
...
\IEEEauthorblockN{
\begin{tabular}{cc}
Kuldeep Singh Thakur & Rahul Dhandore \\
\textit{Dept. of Computer Science and Engineering} &
\textit{Dept. of Data Science and Artificial Intelligence} \\
\textit{IIIT Naya Raipur} &
\textit{IIIT Naya Raipur} \\
Chhattisgarh, India &
Chhattisgarh, India \\
kuldeep23100@iiitnr.edu.in &
rahuld23102@iiitnr.edu.in
\end{tabular}
}
...
\textbf{CLI.} The \texttt{hamhrag} console script exposes \texttt{run},
\texttt{doctor}, \texttt{ingest}, and \texttt{bench} subcommands for batch
processing, diagnostics, automated benchmarking, and CI/CD integration.
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{hamhrag_f1_comparison.png}
  \caption{Token F1 Score comparison on HotpotQA and MuSiQue benchmarks. HAMH-RAG consistently outperforms baselines on both datasets.}
  \label{fig:f1}
\end{figure}
...
\subsection{Comparative Study of Framework Capabilities}
\label{sec:comparative}

To provide a holistic view of how HAMH-RAG advances the state of the art, Table~\ref{tab:comparison} compares its architectural capabilities against prominent existing RAG and multi-hop QA frameworks.

\begin{table*}[!t]
\caption{Comparative Feature Analysis of Multi-Hop RAG Frameworks}
...
\toprule
\textbf{Framework} & \textbf{\shortstack{Dynamic\\Logic-Tree}} & \textbf{\shortstack{Per-Node\\Validation}} & \textbf{\shortstack{Self-Healing\\(Restructuring)}} & \textbf{\shortstack{Pre-Retrieval\\Routing}} & \textbf{\shortstack{Graph+Vector\\Hybrid}} \\
\midrule
Standard RAG \cite{lewis2020retrieval} & Unsupported & Unsupported & Unsupported & Unsupported & Unsupported \\
ReAct \cite{yao2023react} & Not Native (Linear Interleaved) & Unsupported & Implicit (Via LLM prompts) & Unsupported & Unsupported \\
IRCoT \cite{trivedi2023interleaving} & Not Native (Linear Interleaved) & Unsupported & Unsupported & Unsupported & Unsupported \\
CRAG \cite{yan2024corrective} & Unsupported & Explicit (Post-retrieval evaluator) & Implicit (Web search fallback) & Unsupported & Unsupported \\
TreeQA \cite{zhang2025treeqa} & Not Native (Static Semantic Space) & Unsupported & Unsupported & Unsupported & Unsupported \\
\textbf{HAMH-RAG (Ours)} & \textbf{Explicit (Parallelizable)} & \textbf{Explicit (3-Stage Checker)} & \textbf{Explicit (ART-R mutation)} & \textbf{Explicit (Heuristic Scorer)} & \textbf{Explicit (RRF Fusion)} \\
\bottomrule
\end{tabular}
\end{table*}

Unlike ReAct or IRCoT which rely on linear, interleaved reasoning that can silently drift into hallucination, HAMH-RAG explicitly isolates each reasoning step into a tree node. While CRAG introduces an evaluator to trigger fallback searches, it does not decompose queries into multi-hop structures. TreeQA introduces logic-tree constraints but operates on a static semantic space without mid-reasoning grounding validation or self-healing. HAMH-RAG is the first to combine dynamic logic-tree decomposition with a strict per-node validation firewall and structural ART-R self-healing, ensuring that hallucinated intermediate conclusions are detected and corrected before they can corrupt downstream retrieval.
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{hamhrag_latency_chart.png}
  \caption{Retrieval Latency (P50 and P90) across routing strategies, illustrating the 45\% reduction in median latency for predictive parallel fallback.}
