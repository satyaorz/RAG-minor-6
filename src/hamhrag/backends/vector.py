...
class QdrantVectorBackend:
    def __init__(self, url: str, collection_name: str, api_key: str = "") -> None:
        try:
            ...
            raise RuntimeError(
                "qdrant-client is required for HAMHRAG_VECTOR_PROVIDER=qdrant."
            ) from error
...
class LocalVectorBackend:
    ...
    def __init__(
        ...
        if not self.index_path.exists():
            raise RuntimeError(
                f"Local vector index not found at {self.index_path}. Run `python -m hamhrag.cli ingest`."
            )
    ...
    def _build_doc_tokens(self) -> list[set[str]]:
        ...
        from hamhrag.retrieval.scoring import tokenize
...
def build_vector_backend(settings: HamhRagSettings) -> VectorBackend:
    ...
    if provider == "qdrant":
        if not settings.vector_store_url:
            raise ValueError("VECTOR_STORE_URL must be set when HAMHRAG_VECTOR_PROVIDER=qdrant.")
        if not settings.vector_collection:
            raise ValueError(
                "HAMHRAG_VECTOR_COLLECTION must be set when HAMHRAG_VECTOR_PROVIDER=qdrant."
            )
