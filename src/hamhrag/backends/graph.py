...
from hamhrag.retrieval.scoring import lexical_score, normalize_text
...
class MemoryGraphBackend:
    def __init__(self, facts: dict[str, str] | None = None) -> None:
        self.facts = facts or {
            "fact-1": "HamhRag uses logic-tree reasoning to verify intermediate steps.",
...
class Neo4jGraphBackend:
    def __init__(
        ...
        try:
            ...
            raise RuntimeError("neo4j is required for HAMHRAG_GRAPH_PROVIDER=neo4j.") from error
...
class LocalGraphBackend:
    def __init__(self, index_path: str) -> None:
        ...
        if not self.index_path.exists():
            raise RuntimeError(
                f"Local graph index not found at {self.index_path}. Run `python -m hamhrag.cli ingest`."
            )
...
def build_graph_backend(settings: HamhRagSettings) -> GraphBackend:
    ...
    if provider == "neo4j":
        if not settings.graph_store_url:
            raise ValueError("GRAPH_STORE_URL must be set when HAMHRAG_GRAPH_PROVIDER=neo4j.")
...
def _resolve_local_graph_index_path(settings: HamhRagSettings) -> Path:
