...
from hamhrag.state import WorkflowState
...
class HamhRagPipeline:
    def __init__(
        self,
        settings: HamhRagSettings | None = None,
        ...
        self.settings = settings or HamhRagSettings.from_env()
        ...
        *progress_callback* is called with keyword arguments:
        ...
        for tree_attempt in range(tree_retries + 1):
            ...
            if state.root.status == "verified" and state.root.answer:
                ...
                nodes_for_synthesis = state.root.children if state.root.children else verified_nodes
                state.final_answer = self.generator.generate_final(state.query, nodes_for_synthesis, deadline=deadline)
        ...
        if node.children:
            ...
            if do_parallel:
                ...
                for child in node.children:
                    ...
                    if propagate_sibling_context and child.answer:
                        # Always propagate context — even if status is needs_review —
                        # provided the answer actually contains a name/entity.
                        # This prevents node-2 from retrieving docs for the wrong
                        # subject when node-1 fails to verify (e.g. "evidence does
                        # not mention husband") but we still need to pass context.
                        clean_ans = self._strip_sources(child.answer)
                        if child.status == "verified" or self._extract_any_entity(clean_ans):
                            accumulated.append((child.question, clean_ans))
            
            verified_children = [child for child in node.children if child.status in ("verified", "needs_review")]
            node.answer = self.generator.generate_final(node.question, verified_children, deadline=deadline)
    ...
    def _contextualize_question(
        ...
        # --- Resolve placeholders like [husband's name], <director>, {person} ---
        # Matches any text inside <>, [], or {}
        placeholder_match = re.search(r"[<\[\{]([^>\]\}]+)[>\]\}]", question)
        if placeholder_match:
            if entity:
                # Replace the bracketed placeholder with the extracted entity
        ...
        # Replace all anaphoric role references with the extracted entity.
        # This covers: "that person", "that director", "that husband", "that author",
        # water body references, team references, etc.
        _ANAPHORA_PAT = re.compile(
            r"\b(?:this|that|they|their|them|the)\s+"
            r"(?:person|individual|director|author|writer|producer|composer|actor|actress|"
            r"founder|creator|singer|narrator|cinematographer|editor|lyricist|playwright|"
            r"architect|painter|photographer|journalist|pilot|inventor|chairman|president|"
            r"coach|manager|captain|governor|senator|officer|general|professor|"
            r"husband|wife|spouse|partner|sibling|brother|sister|parent|father|mother|"
            r"child|son|daughter|body\s+of\s+water|sea|gulf|bay|strait|lake|river|"
            r"reservoir|baseball\s+team|team|band|group|company|organization|organisation)\b",
            re.IGNORECASE,
        )
        contextualized = _ANAPHORA_PAT.sub(entity, question)
        return contextualized

    @staticmethod
    def _extract_context_entity(answer: str) -> str:
        cleaned = HamhRagPipeline._strip_sources(answer)
        ...
        # Person / role patterns (generalised)
        person_pat = re.search(
            r"(?:"
            r"directed by|written by|composed by|produced by|founded by|created by|"
            r"invented by|discovered by|established by|led by|performed by|sung by|"
            r"authored by|painted by|designed by|built by|governed by|edited by|"
            r"husband(?:\s+(?:is|was|of(?:\s+\w+)?)?)?[:\s]+|"
            r"wife(?:\s+(?:is|was)?)?[:\s]+|"
            r"spouse(?:\s+(?:is|was)?)?[:\s]+|"
            r"married(?:\s+to)?\s+|"
            r"partner(?:\s+(?:is|was)?)?[:\s]+|"
            r"father(?:\s+(?:is|was)?)?[:\s]+|"
            r"mother(?:\s+(?:is|was)?)?[:\s]+|"
            r"son(?:\s+(?:is|was)?)?[:\s]+|"
            r"daughter(?:\s+(?:is|was)?)?[:\s]+|"
            r"director(?:\s+(?:is|was|of(?:\s+the)?(?:\s+film)?)?)?[:\s]+"
            r")"
            r"([A-Z][A-Za-z'.-]+(?:\s+[A-Z][A-Za-z'.-]+){0,4})",
            cleaned,
        )
        if person_pat:
            return person_pat.group(1).strip(" .,;")

        # Fallback: try to extract a capitalized entity
        any_entity = HamhRagPipeline._extract_any_entity(cleaned)
        if any_entity:
            return any_entity

        # Absolute fallback for very short answers (e.g. just a name/place)
        words = cleaned.split()
        if len(words) <= 4:
