...
class QueryDecomposer:
    ...
    _ROLES: tuple[str, ...] = (
        ...
        "narrator", "cinematographer", "editor", "lyricist", "playwright",
        "architect", "painter", "photographer", "journalist", "pilot",
        "inventor", "chairman", "president", "ceo", "coach", "manager",
        "captain", "principal", "dean", "governor", "senator", "mayor",
        "officer", "general", "admiral", "professor", "teacher",
    )

    # Pre-compiled role alternation for faster matching
    _ROLE_ALT: str = "|".join(_ROLES)
    _ROLE_PAT: re.Pattern = re.compile(
        r"\bthe\s+(?:" + _ROLE_ALT + r")\b",
        re.IGNORECASE,
    )
    ...
    def _decompose_event_winner_bridge(self, query: str) -> list[str]:
        ...
        for pat in (
            ...
            if m and "world series" in lowered and "team" in lowered:
                year = m.group(1)
                attr = self._extract_attribute_verb(lowered)
                q2 = f"When was that team {attr}?" if attr else "When was that team established?"
    ...
    def _decompose_role_of_entity_bridge(self, query: str) -> list[str]:
        ...
        Triggers on any question that contains "the ROLE of ENTITY" where
        the question is NOT merely asking *who* that role is (single hop).
        """
        # Step 1: find "the ROLE of" anchor
        role_anchor = re.compile(
            r"\bthe\s+(" + self._ROLE_ALT + r")\s+of\s+" + self._ENTITY_INTRO,
            re.IGNORECASE,
        )
        ...
        for m in _PRED_VERBS.finditer(rest):
            ps = m.start()
        ...
        single_hop = re.match(
            r"^who\s+(is|was|are|were)\s+the\s+(?:" + self._ROLE_ALT + r")\s+of\b",
            query.lower(),
        )
        if single_hop:
            return []
        ...
        q1 = f"Who is the {role} of {entity}?"
    ...
    def _decompose_same_attribute_comparison(self, query: str) -> list[str]:
        ...
        # Must contain "same" and either "and" or "both"
        if "same" not in lowered:
            return []
        if "and" not in lowered and "both" not in lowered:
            return []

        # --- Sub-case A: "Were [Person A] and [Person B] born in the same [attr]?" ---
        m = re.match(
            r"^(?:were|did|do|are|is|have)\s+"
            r"(?P<a>.+?)\s+and\s+(?P<b>.+?)\s+"
            r"(?:born|die|live|come|from|have|share|hold|hold|attend|graduate|study)\b.*"
        ...
        if m:
            ...
            verb = "born in" if "born" in lowered else "from"
            return [
                f"{wh.capitalize()} was {a_entity} {verb}?",
                f"{wh.capitalize()} was {b_entity} {verb}?",
            ]

        # --- Sub-case B: dual-film director nationality ---
    ...
    def _build_tree(self, query: str, parts: list[str]) -> QueryNode:
        ...
        if len(cleaned) == 1:
            sub = cleaned[0].strip(" ?")
            orig = query.strip(" ?")
            # Single leaf: sub-question IS the original → treat as leaf
            if sub.lower() == orig.lower() or orig.lower().startswith(sub.lower()[:50]):
                return QueryNode(node_id="root", question=query.strip())
            # Sub-question DIFFERS from original → make it a child so the
            # root can synthesise the terminal answer (e.g. LLM returned
            # only 1 of the 2 required hops; pipeline will still answer root)
            return QueryNode(
                node_id="root",
                question=query.strip(),
                children=[QueryNode(node_id="node-1", question=cleaned[0])],
            )

        # Multiple sub-questions: root carries original query, children carry hops
        return QueryNode(
            node_id="root",
            question=query.strip(),
            children=[
                QueryNode(node_id=f"node-{i}", question=part)
    ...
    def _extract_dual_titles(self, query: str) -> str | None:
        m = re.search(
            r"both\s+(?:films?|movies?|titles?|works?)?\s*:?(.*?)\s+have\b",
            query,
            flags=re.IGNORECASE,
        )
